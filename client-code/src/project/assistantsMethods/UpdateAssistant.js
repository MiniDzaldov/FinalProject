...
const UpdateAssistant = () => {
  ...
  useEffect(() => {
    const fetchAssistant = async () => {
      try {
        ...
        setValue("id", assistantData.id);
        setValue("firstName", assistantData.firstName);
        setValue("lastName", assistantData.lastName);
        setValue("email", assistantData.email);
        setValue("phoneNumber", assistantData.phoneNumber);
        setValue("categoryCode", assistantData.categoryCode);
        setValue("street", assistantData.addressCodeNavigation.street);
        setValue("city", assistantData.addressCodeNavigation.city);
        setValue("numOfBuilding", assistantData.addressCodeNavigation.numOfBuilding);
  ...
  const onSubmit = async (formData) => {
    try {
      ...
      const response = await axios.put(`http://localhost:5089/api/assistants/${formData.id}`, dataToSend);
